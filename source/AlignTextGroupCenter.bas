...
Public Sub AlignTextGroupCenter()
    ...
    ' 設定値読み込み
    configData = CommitConfig.LoadConfig(FormDisplay.ALIGNTEXT_CONFIG)
    ...
    If CommonFunction.IsTextObject(targetEntity) Then
        Call alignTextCenter(targetEntity, pickPoint, configData)
    ElseIf TypeOf targetEntity Is ZcadBlockReference Then
        Call alignBlockCenter(targetEntity, pickPoint, configData)
    Else
...
Private Sub alignTextCenter(ByRef target_text As ZcadEntity, _
                            ...
                            ByVal config_options As String)
    
    On Error GoTo Error_Handler
    
    Dim firstPoint As Variant, secondPoint As Variant
    Dim offsetFactor As Single
    Dim underFlag As String
    Dim textCenter() As Double
    
    ' 調整値のユーザー入力
    Call inputAlignValue _
        (firstPoint, secondPoint, offsetFactor, underFlag, config_options)
...
Private Sub inputAlignValue(ByRef first_point As Variant, _
                            ...
                            ByRef offset_factor As Single, _
                            ByRef under_flag As String, _
                            ByVal config_options As String)
    ...
    offset_factor = ThisDrawing.Utility.GetInteger _
        ("オフセット係数(x/10)を入力 または [" & config_options & "]:")
